...
class RMS:
    def __init__(self, root):
        self.root = root
        self.root.title("RESULT MANAGENMENT SYSTEM")
        self.root.geometry("1000x500+170+150")
        self.root.config(bg="white")
        self.root.focus_force()
        # =========title=================
        title = Label(self.root, text=" R E S U L T S   M A N A G E M E N T   S Y S T E M ", font=("gaudy old style", 25, "bold"), bg="#033054",
        ...
        combo_sem = ttk.Combobox(Manage_Frame, textvariable=self.sem_var, font=("times new roman", 10, "bold"),
                                 state="readonly")
        combo_sem['values'] = ("sem1", "sem2", "sem3", "sem4", "sem5", "sem6")
        combo_sem.grid(row=2, column=1, pady=10, padx=20, sticky="w")
        # result
        lbl_report = Label(Manage_Frame, text="Result", bg="white", fg="black",
                           font=("times new roman", 10, "bold"))
        lbl_report.grid(row=3, column=0, pady=10, padx=20, sticky="w")

        self.txt_rep = Entry(Manage_Frame, textvariable=self.result_var, font=("times new roman", 10, "bold"), bd=5,
                             relief=GROOVE)
        self.txt_rep.grid(row=3, column=1, pady=10, padx=20, sticky="w")
        # ==============button================================================
        btn_Frame = Frame(Manage_Frame, bd=4, relief=RIDGE, bg="white")
        btn_Frame.place(x=100, y=300, width=250, height=70 )

        updatebtn = Button(btn_Frame, text="UPDATE", width=13,height=2,bg="#006600",fg="white",command=self.update_stdnt, font=("gaudy old style", 9, "bold")).grid(row=0, column=0, padx=10,pady=10)
        clearbtn = Button(btn_Frame, text="CLEAR", width=13, height=2,command=self.clear, bg="#666699", fg="white", font=("gaudy old style", 9, "bold")).grid(row=0, column=1, padx=10, pady=10)
    ...
    def update_stdnt(self):
        ...
        try:
            if self.Roll_No_var.get() == "":
                ...
                cur.execute("select * from result where roll=%s", (self.Roll_No_var.get()))
                row = cur.fetchone()
                if row is None:
                    messagebox.showerror("Error", "put the correct roll no", parent=self.root)
                else:
                    cur.execute(
                        "update result set " + str(self.sem_var.get()) + "=%s where roll=%s",
                        (
                            self.result_var.get(),
    ...
    def search(self):
        con = pymysql.connect(host="localhost", user="root", password="1234", database="sms")
        cur = con.cursor()
        cur.execute("select * from result where roll LIKE '%" + str(self.search_var.get()) + "%' ")
        rows = cur.fetchall()
